...
- [ ] [FEAT] 설정에 "추첨 방식: 전원 / 선호" 토글을 넣고, 선호 모드에서 상품 추첨이 `candidatesFor` 결과로 원판을 구성하도록 배선. 후보 0명 폴백 시 화면에 배지 표시, 상품별 후보 수 노출. 이 단계의 `picks`는 아직 수동 주입(개발 시드)으로 동작. 전원 모드 동작·저장은 무변경. 근거: `docs/design/preference-draw-mode.md`. 검증: 스핀 후 "포인터 아래 이름 == 표시된 당첨자" 브라우저 확인, 두 모드 왕복.
