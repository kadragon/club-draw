...
4. 행사 후 **서버에서 세션 삭제**(되돌릴 수 없음) → 명단·선택 행 삭제, `/pick?s=`는 404. 로컬 picks는 남는다.
5. 세션 초기화·백업 복원은 로컬 `state.session`(운영자 토큰)을 비운다 → 서버 세션이 열려 있거나
   삭제 전이면 **먼저 마감·삭제**하고 초기화할 것(토큰을 잃으면 서버 쪽 정리 경로가 없다).
...
| 마감·pull·삭제 401 | 운영자 토큰 불일치/분실(초기화·복원·다른 브라우저). 서버 쪽 복구 경로 없음 — 보관한 토큰 필요. |
