...
크림빛 캔버스(`--canvas` #fffaf0) 위에 near-black 잉크, 고대비(빔 대응) 원판, 따뜻한 그라데이션 메시.
...
- 채도/명도 3밴드 회전(`sat=[0.72,0.85,0.78]`, `light=[0.45,0.52,0.37]`)으로 근접 색조 분리.
- **프로젝터 계약**: 빔에서 색이 흰색으로 수렴하지 않도록 `light`를 중간-어둡게 유지.
  잉크(`LABEL_INK`) 선택 임계는 WCAG 수학적 교차점 `lum > 0.186` — 어두운/흰 잉크 중
  더 높은 대비비를 항상 선택. 밴드를 재튜닝하면 `test/wheel-palette.test.ts` 대비비 가드 확인.
